...
fun AppNavHost() {
    ...
    CompositionLocalProvider(LocalAppSnackbarHost provides snackbarHost) {
        Scaffold(snackbarHost = { SnackbarHost(snackbarHost) }, bottomBar = {
            if (route in bottom) NavigationBar {
                bottom.forEach { destination ->
                    val label = when (destination) { "home" -> stringResource(R.string.app_name); "downloads" -> stringResource(R.string.downloads); "library" -> stringResource(R.string.library); else -> stringResource(R.string.settings) }
                    NavigationBarItem(selected = route == destination, onClick = { navController.navigate(destination) }, icon = { Text(label.take(1)) }, label = { Text(label) })
                }
            }
        }) {
